...
if __name__ == '__main__':
    ...
    class Net(torch.nn.Module):  # 继承 torch 的 Module
        def __init__(self):
            ...
            self.predict = torch.nn.Linear(128, 48, bias=False)   # 输出层线性输出
    ...
    class Net_sae(torch.nn.Module):  # 继承 torch 的 Module
        def __init__(self):
            ...
            # 定义每层用什么样的形式
            self.sae_hidden0 = torch.nn.Linear(3, 256, bias=True)   
            self.sae_hidden1 = torch.nn.Linear(256, 128, bias=True)
            self.sae_hidden2 = torch.nn.Linear(128, 64, bias=True)
            self.sae_hidden3 = torch.nn.Linear(64, 128, bias=True)
            self.sae_hidden4 = torch.nn.Linear(128, 256, bias=True)
            self.sae_predic = torch.nn.Linear(256, 3, bias=True)
    ...
    def test_val0() :
        ...
        loss = loss_func(prediction,torch.max(test_y, 1)[0])
    ...
    loss_func = torch.nn.NLLLoss()      # 预测值和真实值的误差计算公式 (均方差)
    ...
    for epoch in range(500):   # NET
        ...
        for (batch_x, batch_y) in loader:
            ...
            loss = loss_func(prediction,torch.max(b_y, 1)[0])
    ...
    net_sae_net = torch.nn.Sequential(
        ...
        torch.nn.ReLU(),
        torch.nn.Linear(256, 48, bias=True),
        torch.nn.Softmax(), 
    )
